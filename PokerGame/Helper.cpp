...
string Helper::sImageCard(const card& c)const{
    if(c.first<10){

        cout<< PREFIX+"0"+to_string(c.first)+to_string(c.second)+SUFFIX;
        return PREFIX+"0"+to_string(c.first)+to_string(c.second)+SUFFIX;
    }
    else{
        cout<< PREFIX+to_string(c.first)+to_string(c.second)+SUFFIX;
        return PREFIX+to_string(c.first)+to_string(c.second)+SUFFIX;
    }
}
