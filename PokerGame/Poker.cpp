...
void Poker::initializeGame(){
    ...
    messages.insert({static_cast<short>(MESSAGE::TIE),"You tie"});
...
deque<string> Poker::getImages(int role) const{
    deque<string> tmp;
    string image;
    set<card> cards = getCards(role);
...
bool Poker::setDiscardedPoss(int num){

    // save positions to discardedPos
    player.setDiscardedPoss(num);
...
short Poker::update(short currentState){


    if(currentState == static_cast<short>(GAME_STATE::START)){
        state = static_cast<short>(GAME_STATE::DEAL);
        status = static_cast<short>(GAME_STATUS::PLAYING);
        dealCard();
        rankHand();
    ...
    if(currentState == static_cast<short>(GAME_STATE::DRAW)){
        drawCard();
        rankHand();
        score();
        state = static_cast<short>(GAME_STATE::SHOW);
    ...
    if(currentState == static_cast<short>(GAME_STATE::SHOW)){
        score();
        state = static_cast<short>(GAME_STATE::REPLAY);
        return state;
    }
...
string Poker::getResult(){
    if(dealer.getScore() < player.getScore()){
        return messages[static_cast<short>(MESSAGE::WIN)];
    }
    else if(dealer.getScore()> player.getScore()){
        return messages[static_cast<short>(MESSAGE::LOOSE)];
    }
    else
        return messages[static_cast<short>(MESSAGE::TIE)];
}
